...
#' Age gender Bayesian hierarchical logistic regression with Stan
#'
#' @param age Numeric vector of age values.
#' @param sex_cats integer value of total number of sexes possible (in this case it's 2).
#' @param sex_idx integer value identifying the passenger sex.
...
#' @export
#'
age_gender_hier_stan <- function(age, sex_cats, sex_idx, survived, test_sex_idx, test_age, ...) {
  standata <- list(N=length(survived),
                   age=age,
                   sex=sex_cats,
                   sex_idx=sex_idx,
                   ...
                   test_age=test_age,
                   test_sex_idx=test_sex_idx)
  out <- rstan::sampling(stanmodels$age_gender_hier_model, data = standata, ...)
  return(out)
}
